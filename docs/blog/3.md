---
title: MacOS 中 Oh-My-Zsh 的安装使用
tags:
  - Git
---
# MacOS 中 Oh-My-Zsh 的安装使用
...
::: tip 注意
最新的 M1 芯片的系统可能会出现直接执行远程的 shell 脚本报错的情况。可以通过把脚本下载到本地的方式进行执行。
:::

1. 先直接打开脚本的地址 [Oh My Zsh 脚本](https://raw.github.com/ohmyzsh/ohmyzsh/master/tools/install.sh)，拷贝文本（你也可以直接拷贝下面我贴的 shell 脚本）。保存在一个后缀为 `.sh`的文档中。例如 `oh-my-zsh.sh`。
1. 在本地终端中执行脚本 `sh oh-my-zsh.sh`
...
![](./attachments/21a688a5-2568-4597-a76d-c8a26bca1c0b.png)
...
路径请自行修改。最后别忘了修改 `~/.zshrc` 文件的主题配置并 `source` 一下。
